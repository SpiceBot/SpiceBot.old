...
def bot_setup_users(bot, trigger):
    ...
    if bot_startup_requirements_met(bot, ["users"]):
        return
    ...
    for channel in bot.memory["botdict"]["tempvals"]["servers_list"][currentservername]['channels_list'].keys():
        ...
        for identchannel in bot.channels.keys():
            if str(identchannel).lower() == str(channel).lower():
                channelident = identchannel
        for user in bot.channels[channelident].privileges.keys():
            ...
            try:
                userprivdict[str(user)] = bot.channels[channelident].privileges[user] or 0
            except KeyError:
