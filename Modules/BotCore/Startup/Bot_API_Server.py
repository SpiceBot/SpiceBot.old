...
def api_socket_server(bot, trigger):
    ...
    if bot_startup_requirements_met(bot, ["bot_api"]):
        return
...
def api_socket_setup(bot):
    ...
    if bot_startup_requirements_met(bot, ["monologue"]):
        osd(bot, bot.channels.keys(), 'say', "API Port set to " + str(bot.memory['sock_port']))
                    ...
                    if spicemanip.main(str(data), 1) == "GET":
                        ...
                        if jsondict["type"] == "message":
                            ...
                            if jsondict["command"] == 'update':
                                ...
                                if sender != "API":
                                    osd(bot, bot.channels.keys(), 'say', "Received API command from " + sender + " to update from Github and restart. Be Back Soon!")
                                else:
                                    osd(bot, bot.channels.keys(), 'say', "Received API command to update from Github and restart. Be Back Soon!")
                                ...
                                if sender != "API":
                                    osd(bot, bot.channels.keys(), 'say', "Received API command from " + sender + " to restart. Be Back Soon!")
                                else:
                                    osd(bot, bot.channels.keys(), 'say', "Received API command to restart. Be Back Soon!")
