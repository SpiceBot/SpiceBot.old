...
def bot_api_fetch_tcp(bot, TCP_PORT, TCP_IP):
    ...
    try:
        botdict_return = json.loads(data.decode('utf-8'), object_hook=json_util.object_hook)
...
def bot_api_fetch_web(bot, botport, host):
    ...
    # examine results
    result = page.content
    botdict_return = json.loads(result.decode('utf-8'), object_hook=json_util.object_hook)
...
def bot_dictcom_feeds_handler(bot, feed, forcedisplay):
    ...
    if feed_type in ['rss', 'youtube', 'github', 'redditrss']:
        ...
        http_auth = bot.memory["botdict"]["tempvals"]['googlecal'].authorize(Http())
