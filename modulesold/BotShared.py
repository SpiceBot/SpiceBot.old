...
def bot_command_users(bot, botcom):
    ...
    for user in botcom.users_current:
        ...
        for channelcheck in bot.channels:
            try:
                if bot.channels[channelcheck].privileges[user] == OP:
                    botcom.chanops.append(user)
                    botcom.opadmin.append(user)
            except KeyError:
                dummyvar = 1
            try:
                if bot.channels[channelcheck].privileges[user] == VOICE:
    ...
    for channel in bot.channels:
        for u in bot.users:
            ...
            if u != bot.nick:
                ...
                try:
                    if bot.channels[channel.lower()].privileges[u.lower()] == OP:
                        operatorarray.append(u)
                except KeyError:
                    dumbyvar = 1

                try:
                    if bot.channels[channel.lower()].privileges[u.lower()] == VOICE:
                        voicearray.append(u)
                except KeyError:
                    dumbyvar = 1
...
def randomline(bot, address):
    ...
    htmlfile = urllib.request.urlopen(address)
