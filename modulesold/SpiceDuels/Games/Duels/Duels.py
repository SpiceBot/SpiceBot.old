...
def setup(bot):
    ...
    bot.memory['duels'] = dict()

    bot.memory['duels']['message_display'] = dict()
    bot.memory['duels']['message_display']["used_ids"] = [0]
...
# bot.nick do this
@module.nickname_commands('duel', 'challenge', 'duels', 'challenges')
@module.thread(True)
def duel_nickcom(bot, trigger):
    # command_type = 'botnick'
    onscreendisplay(bot, trigger.sender, 'say', "Don't tell me what to do!")
...
def execute_main(bot, trigger, triggerargsarray, command_type):
    ...
    duels.messagesid = unique_id_create(bot)
    messagelog_start(bot, duels.messagesid)
    ...
    if not command_full:
        if duels.command_type != 'actionduel':
            messagelog_error(bot, duels.messagesid, 'specify_command_target')
        else:
            messagelog_error(bot, duels.messagesid, 'specify_command')
        return messagelog_exit(bot, duels, duels.messagesid)
    if duels.command_type == 'actionduel':
        if "&&" in command_full:
            messagelog_error(bot, duels.messagesid, 'multi_action_com')
            return messagelog_exit(bot, duels, duels.messagesid)
    ...
    if duels.duels_enabled_channels == []:
        if not trigger.admin:
            messagelog_error(bot, duels.messagesid, 'duels_not_enabled_any')
            return messagelog_exit(bot, duels, duels.messagesid)
    if duels.channel_current not in duels.duels_enabled_channels and duels.inchannel:
        if not trigger.admin:
            messagelog_error(bot, duels.messagesid, 'duels_not_enabled_current')
            return messagelog_exit(bot, duels, duels.messagesid)
    ...
    for command_split_partial in commands_array:
        ...
        if [x for x in triggerargsarray_part if x == "-a"]:
            ...
            if not trigger.admin:
                messagelog_error(bot, duels.messagesid, 'admin_switch_admin')
                continue
...
@module.rule('.*')
@module.thread(True)
def duels_chanceevents(bot, trigger):
    while True:
...
@module.rule('.*')
@module.thread(True)
def duels_halfhourtimer(bot, trigger):
    while True:
...
def duels_user_lists(bot, duels):
    ...
    if not len(bot.channels.keys()) > 0:
    ...
    for channel in bot.channels:
        ...
        # chan op
        for user in users_current_channel:
            if bot.channels[channel].privileges[user] == module.OP:
                if user not in duels.chanop:
                    duels.chanop.append(user)

        # chan voice
        for user in users_current_channel:
            if bot.channels[channel].privileges[user] == module.VOICE:
...
@module.rule('.*')
@module.thread(True)
def duels_merchant_restock(bot, trigger):
    while True:
        restock_last_timesince = duels_time_since(bot, 'duelrecorduser', "restock_last_time") or 0
        if restock_last_timesince > 3600:
            ...
            now = time.time()
            set_database_value(bot, 'duelrecorduser', "restock_last_time", now)
...
def duels_nick_titles(bot, playerbio, duels):
    ...
    try:
        ...
        # OP
        elif bot.channels[duels.channel_current.lower()].privileges[playerbio.actual.lower()] == module.OP:
            nickname = duels_custom_title_chan_op
        # VOICE
        elif bot.channels[duels.channel_current.lower()].privileges[playerbio.actual.lower()] == module.VOICE:
...
def onscreendisplay(bot, recipients, text_type, messages):
    osd(bot, recipients, text_type, messages)


def osd(bot, recipients, text_type, messages):
    return bot.osd(messages, recipients, text_type)
...
def unique_id_create(bot):
    unique_id = 0
    while unique_id in bot.memory['duels']['message_display']["used_ids"]:
        unique_id = uuid.uuid4()
    bot.memory['duels']['message_display']["used_ids"].append(unique_id)
    return unique_id
...
"""
duels Message Handling
"""


def messagelog_start(bot, log_id):

    bot.memory['duels']['message_display'][log_id] = []
...
def messagelog_error(bot, log_id, error_id):
    ...
    for existing_messagedict in bot.memory['duels']['message_display'][log_id]:
    ...
    bot.memory['duels']['message_display'][log_id] = newloglist
...
def messagelog(bot, log_id, recipients, message):
    ...
    bot.memory['duels']['message_display'][log_id].append(messagedict)


def messagelog_exit(bot, duels, log_id):

    current_messages = []
    current_errors = []

    for messagedict in bot.memory['duels']['message_display'][log_id]:
        ...
        if messagedict["type"] == "error":
            ...
            message = messagelog_fillin(bot, duels, message)
    ...
    for messagedict in current_messages:
        if messagedict["type"] == 'error':
            osd(bot, duels.instigator, 'notice', messagedict['message'])
        else:
            osd(bot, messagedict["recipients"], 'say', messagedict['message'])

    del bot.memory['duels']['message_display'][log_id]


def messagelog_fillin(bot, duels, message):

    if "$channel_current" in message:
        if duels.inchannel:
            message = str(message.replace("$channel_current", duels.channel_current))
        else:
            message = str(message.replace("$channel_current", 'privmsg'))
